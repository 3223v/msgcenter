...
import org.mttk.msgcenter.mapper.GlobalQuotaMapper;
import org.mttk.msgcenter.mapper.SourceQuotaMapper;
import org.mttk.msgcenter.model.entity.GlobalQuotaModel;
import org.mttk.msgcenter.model.entity.SourceQuotaModel;
...
import static org.mttk.msgcenter.constant.Constants.REDIS_KEY_RATE_LIMIT_COUNT;
import static org.mttk.msgcenter.constant.Constants.REDIS_KEY_RATE_LIMIT_COUNT_TIMER;
...
public class RateLimitServiceImpl implements RateLimitService {
    ...
    private String getQuotaConfWithCache(int channel,String sourceId,boolean isTimerMsg) {
        ...
        if(!StringUtils.isEmpty(cacheQt) && sendMsgConf.isOpenCache()){
            ...
            //先从渠道限额获取配置
            LambdaQueryWrapper<SourceQuotaModel> wrapper = Wrappers.lambdaQuery();
            wrapper.eq(SourceQuotaModel::getSourceId, sourceId);
            wrapper.eq(SourceQuotaModel::getChannel, channel);
            SourceQuotaModel sourceQuota = sourceQuotaMapper.selectOne(wrapper);
...
//                GlobalQuotaModel globalQuota = globalQuotaMapper.getGlobalQuota(channel);
                LambdaQueryWrapper<GlobalQuotaModel> wp = Wrappers.lambdaQuery();
                wp.eq(GlobalQuotaModel::getChannel, channel);
                GlobalQuotaModel globalQuota = globalQuotaMapper.selectOne(wp);
                if (globalQuota != null){
